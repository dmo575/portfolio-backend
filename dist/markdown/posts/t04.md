...
- **Understanding the final build** *-> Relationship between servlet, WAR, servers...*
...
- Spring Boot application: A project configured for you by Spring's Initializr.
	...
	- It follows the convention over configuration rule, like Maven (We will see this at play with MVC). Which also means that initial configurations are opinionated.
...
**To put it simply: ResponseEntity > annotations > servlet.**
...
- A reference to the servlet response that sets the status code to: 202 (ACCEPTED)
- An instance of `ResponseEntity` that sets the status code to: 201 (CREATED)
- A write operation to the response body via the servlet reference.

What do you think the status code will be?

It will be 201 (CREATED) coming from `ResponseEntity`.
...
We talked about the order of precedence when it came to edits to the servlet object earlier on:
- ResponseEntity > Annotations > Servlet

But there is another order of precedence that has to do with **endpoint definition**. Meaning that whenever Spring is trying to figure out an endpoint, it will follow this order:
