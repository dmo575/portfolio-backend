...
public class MyClass {
	...
	public ResponseEntity<String> index() {
		...
		return new ResponseEntity<String>(str, headers, HttpStatusCode.valueOf(200));
	}
}
```
...
```
Note that while we manage to pack the status code and the return body in a single line, we have no way of adding a header. These methos are meant to create quick response HTTP objects and only allow you to pass in the body contents.
...
To put it simply:
- **Body**: *ResponseEntity **>** HttpResponseServlet*
	- An error is thrown if you try to add a body to both of them (`getWriter() has already been called for this response`)
- **Headers**: *ResponseEntity **+** HttpResponseServlet*
	- Both headers make it to the HTTP response.
- **Status code**: *ResponseEntity **>** annotations **>** HttpResponseServlet*
	- The one with the highest priority of those gets to make it to the HTTP response.
...
public ResponseEntity<String> index(HttpServletResponse res) throws IOException {
	...
	return new ResponseEntity<String>("Entity to body", entityHeaders, HttpStatusCode.valueOf(201));
}
```
...
We mentioned this separately in the "HTTP" and "The basics" sections but for the sake of having it in the same place:

Spring can JSONify things into the response body for you. You have two ways of doing this:
- Using `ResponseEntity<T>`: The type T tells Spring the object you intend on sending to the body as a JSON. (You can use `<?>` if you don't know beforehand).
- Using `@ResponseBody`: This signals to Spring that the return type of your method is to be serialized as a JSON into the response body.

In the REST example code, the animal instance would get serialized into the body of the response HTTP. 
...
Now, because this is Spring Boot, by including this dependency we ensure all the configuration is done for us, so we don't have to worry about that.
