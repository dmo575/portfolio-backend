...
We also won't talk about what Spring or how to use it. We may mention a couple of things here and there but you should be somewhat familiar with the Spring framework; beans, factories, IoC and all the rest.
...
- The only of the three that we will explain a bit of is **MVC**. We will brush over the theory before implementing it.
...
- Use `@Controller`
- Use `@RequestMapping`
- Use `@RequestBody` on the endpoind methods. This will let Spring know that whatever you return is to be placed in the body of the HTTP response.
