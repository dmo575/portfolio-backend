...
Now, for some reason when I first took a look at that report I decided to just look at the `signal=KILL` and then inmediatelly jumped out of there to check things like the memory space and system logs. It's only later that I decided to go back there and take a better look.
...
If you peek there you can see a couple of instances where I get messages similar to this one: `Connection closed by invalid user pi 95.223.184.224 port 12868 [preauth]`. You can see a couple of those happening less than a second from each other. You can also see things like `CRYPTO_KEY_USER` in there.
