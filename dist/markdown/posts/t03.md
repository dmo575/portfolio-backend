...
My inmediate thought as a layman on after seeing these things was "Oh wow, could someone have successfully logged in and it is now using my server to mine crypto or something?"
...
After looking around I found out that I can turn off the server via my DigitalOcean account. I ended up doing that because I had lost access to the server. It took a while for the process to finish but after it did I finally turned it on again.

I tried connecting to the server again and it worked. I then tried the `dnf makecache` command again and magically it worked too. So that was it; I had fixed the issue. I had no idea why it happened in the first place, but I had fixed it.
